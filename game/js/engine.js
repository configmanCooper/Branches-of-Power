...
var Engine = (function() {
    ...
    function createInitialState(gameLength) {
        ...
        state = {
            ...
            supremeCourt: {
                ...
                writeOpinionUsedThisRound: false
            },
    ...
    function expireOldDeals() {
        for (var i = 0; i < state.deals.length; i++) {
            ...
            if (d.status === 'accepted' && d.round < state.round - 1) {
                // Deal breaker penalty and victim sympathy before breaking
                state[d.from].vp = Math.max(0, state[d.from].vp - 2);
                state[d.to].vp += 1;
                addLog(d.from, 'Deal Expired', 'Failed to fulfill deal with ' + Config.ROLE_LABELS[d.to] + '. -2 VP.');
                addLog(d.to, 'Sympathy', 'Deal broken by ' + Config.ROLE_LABELS[d.from] + '. +1 VP sympathy.');
                breakDeal(d.id);
            }
    ...
    function presidentAssignJustice(leaning) {
        ...
        if (state.president.totalNominations >= 4) {
            return { success: false, message: 'Maximum 4 nominations per game.' };
        }
        ...
        state.president.totalNominations++;
    ...
    function housePassBill(pcToUse) {
        ...
        if (votes >= threshold) {
            ...
            // Congressional Fatigue: diminishing returns on repeated bill passage
            var fatigue = Math.min(state.house.billsPassedTotal || 0, 1);
            housePassVP = Math.max(1, housePassVP - fatigue);
            ...
            // Track total bills passed (after fatigue calculation uses pre-increment value)
    ...
    function courtClerksResearch() {
        ...
        state.supremeCourt.jp += 2;
        addLog('supremeCourt', 'Clerks Research', '+2 JP.');
        advanceTurn();
        return { success: true, message: 'Clerks completed research. +2 JP.' };
    }

    // Write Opinion — +2 VP, +1 JP. Once per round.
    function courtWriteOpinion() {
        if (state.supremeCourt.writeOpinionUsedThisRound) return { success: false, message: 'Already used this round.' };
        state.supremeCourt.writeOpinionUsedThisRound = true;
        state.supremeCourt.vp += 1;
        state.supremeCourt.jp += 1;
        addLog('supremeCourt', 'Write Opinion', '+1 VP, +1 JP.');
        advanceTurn();
        return { success: true, message: 'Opinion written. +1 VP, +1 JP.' };
    }
    ...
    function resolveUnitySummit(triggerRole) {
        ...
        // Track participants for end-game scoring
        state.unitySummitParticipants = summit.agreed.slice();
        state.pendingUnitySummit = null;
        advanceTurn();
    ...
    function endRound() {
        ...
        if (state.round > state.maxRounds) {
            // Court Legacy: SC earns 1 VP per judicial review outcome (precedent)
            if (state.precedents && state.precedents.length > 0) {
            ...
            // Legislative Legacy: +3 VP for role with most bills passed
            var houseBills = state.house.billsPassedTotal || 0;
            var senateBills = state.senate.billsPassedTotal || 0;
            ...
            // Diplomat: +3 VP for role with most deals fulfilled
            if (state.deals && state.deals.length > 0) {
                var dealCounts = { president: 0, house: 0, senate: 0, supremeCourt: 0 };
                for (var di = 0; di < state.deals.length; di++) {
                    if (state.deals[di].status === 'fulfilled') {
                        dealCounts[state.deals[di].from]++;
                    }
                }
                var maxDeals = 0;
                Config.ROLES.forEach(function(r) { if (dealCounts[r] > maxDeals) maxDeals = dealCounts[r]; });
                if (maxDeals > 0) {
            ...
            // Crisis Manager: +3 VP for role that resolved most events
            if (state.eventHistory && state.eventHistory.length > 0) {
                var resolveCounts = { president: 0, house: 0, senate: 0, supremeCourt: 0 };
                for (var ei = 0; ei < state.eventHistory.length; ei++) {
                    var evt = state.eventHistory[ei];
                    if (evt && evt.resolvedBy) {
                        resolveCounts[evt.resolvedBy]++;
                    }
                }
                var maxResolved = 0;
                Config.ROLES.forEach(function(r) { if (resolveCounts[r] > maxResolved) maxResolved = resolveCounts[r]; });
                if (maxResolved > 0) {
                    Config.ROLES.forEach(function(r) {
            ...
            // Stability Champion: +3 VP for all Unity Summit participants if stability >= 8
        ...
        state.supremeCourt.writeOpinionUsedThisRound = false;
        ...
        if (state.activeEvent && !state.activeEvent.resolved) {
            ...
            if (state.activeEvent.roundsRemaining <= 0) {
                ...
                state.eventHistory.push({ id: failEvt.id, resolvedBy: null });
    ...
    function getAvailableActions(role) {
        ...
        switch (role) {
            case 'president':
                ...
                if (state.pendingJustice && state.president.justiceNominationsThisRound < 2 && state.president.totalNominations < 4) {
            ...
            case 'house':
                ...
                if (!state.house.initiatedLegislationThisRound) {
                    actions.push({ id: 'initiateLegislation', label: 'Initiate Legislation', cost: 'Free', description: 'New bill on floor' });
                }
            ...
            case 'supremeCourt':
                ...
                // Clerks Research — +2 JP, +1 VP
                if (!state.supremeCourt.clerksResearchUsedThisRound) {
                    actions.push({ id: 'clerksResearch', label: 'Clerks Research', cost: 1, description: '+2 JP, +1 VP' });
                }
                // Write Opinion — +2 VP, +1 JP
                if (!state.supremeCourt.writeOpinionUsedThisRound) {
                    actions.push({ id: 'writeOpinion', label: 'Write Opinion', cost: 1, description: '+1 VP, +1 JP' });
                }
    ...
    function executeAction(role, actionId, params) {
        ...
        switch (actionId) {
            ...
            case 'writeOpinion': actionResult = courtWriteOpinion(); break;
                ...
                if (!state.activeEvent) {
                    ...
                    if (!resolution) {
                        ...
                        if (actionResult.resolved) {
                            ...
                            state.eventHistory.push({ id: state.activeEvent.id, resolvedBy: role });
                            state.activeEvent = null;
                            if (state.queuedEvent) {
                                state.activeEvent = state.queuedEvent;
                                state.queuedEvent = null;
