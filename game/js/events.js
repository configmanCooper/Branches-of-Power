...
var GameEvents = (function() {
    ...
    // Catch-up events — triggered when VP gap > 20
    var CATCHUP_EVENTS = [
        {
            id: 'publicBacklash',
            ...
            category: 'Catchup',
            severity: 2,
            flavor: 'The public is outraged at the concentration of power. The leading branch faces intense scrutiny.',
            deadline: 1,
            triggerCondition: null,
            failPenalty: { stability: -1, vp: { all: 0 }, popularity: 0 },
            resolutions: [
                {
                    id: 'backlashAccept',
                    label: 'Accept Criticism',
                    description: 'Auto-resolves: leader loses 3 VP, others gain 1 VP',
                    roles: ['president', 'house', 'senate', 'supremeCourt'],
                    cooperative: false,
                    cost: { actions: 0, pc: 0, vp: 0, popularity: 0, jp: 0 },
                    reward: { stability: 0, vp: {} },
                    rollRequired: false,
                    agreedRoles: []
                }
            ],
        ...
        },
        {
            ...
            category: 'Catchup',
            severity: 1,
            flavor: 'A grassroots political movement rallies behind the underdog branch.',
            deadline: 1,
            triggerCondition: null,
            failPenalty: { stability: 0, vp: { all: 0 }, popularity: 0 },
            resolutions: [
                {
                    id: 'grassrootsRally',
                    label: 'Rally Support',
                    description: 'Auto-resolves: last place gets +4 VP',
                    roles: ['president', 'house', 'senate', 'supremeCourt'],
                    cooperative: false,
                    cost: { actions: 0, pc: 0, vp: 0, popularity: 0, jp: 0 },
                    reward: { stability: 0, vp: {} },
        ...
        },
        {
            ...
            category: 'Catchup',
            severity: 1,
            flavor: 'Calls for political reform sweep the nation, benefiting those who have fallen behind.',
            deadline: 1,
            triggerCondition: null,
            failPenalty: { stability: 0, vp: { all: 0 }, popularity: 0 },
            resolutions: [
                {
                    ...
                    description: 'Auto-resolves: all trailing players get +2 VP',
                    roles: ['president', 'house', 'senate', 'supremeCourt'],
                    cooperative: false,
                    cost: { actions: 0, pc: 0, vp: 0, popularity: 0, jp: 0 },
                    reward: { stability: 0, vp: {} },
    ...
    function getRandomEvent(state) {
        ...
        var recentIds = recentHistory.map(function(e) { return typeof e === 'string' ? e : e.id; });
        var eligible = [];
        for (var i = 0; i < EVENT_POOL.length; i++) {
            var evt = EVENT_POOL[i];
            // Skip if in recent history
            if (recentIds.indexOf(evt.id) !== -1) continue;
    ...
    function checkEventTrigger(state) {
        // Catch-up event trigger: if VP gap > 20, trigger a catch-up event and apply immediately
        var roles = ['president', 'house', 'senate', 'supremeCourt'];
        var maxVP = -Infinity, minVP = Infinity, leaderRole = null, lastRole = null;
        for (var vi = 0; vi < roles.length; vi++) {
            var rvp = state[roles[vi]].vp;
            if (rvp > maxVP) { maxVP = rvp; leaderRole = roles[vi]; }
            if (rvp < minVP) { minVP = rvp; lastRole = roles[vi]; }
        }
        if (maxVP - minVP > 20 && state.round > 2) {
            var catchupEvent = CATCHUP_EVENTS[Math.floor(Math.random() * CATCHUP_EVENTS.length)];
            // Apply effect immediately instead of queuing
            if (catchupEvent.id === 'publicBacklash') {
                state[leaderRole].vp -= 3;
                for (var pi = 0; pi < roles.length; pi++) {
                    if (roles[pi] !== leaderRole) state[roles[pi]].vp += 1;
                }
            } else if (catchupEvent.id === 'grassrootsMovement') {
                // Give +4 VP to all players tied at last place
                for (var gi = 0; gi < roles.length; gi++) {
                    if (state[roles[gi]].vp === minVP) state[roles[gi]].vp += 4;
                }
            } else if (catchupEvent.id === 'reformWave') {
                for (var ri = 0; ri < roles.length; ri++) {
                    if (state[roles[ri]].vp < maxVP) state[roles[ri]].vp += 2;
                }
            }
            // Don't actually queue as active event — just apply and note in history
            return null;
        }
